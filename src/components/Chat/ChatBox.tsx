...
function ChatBox(props: ChatBoxData) {
  ...
  EchoClient.channel(`chatmessage${sort.toString()}`).listen(
    "ChatSendEvent",
    (data: { data: ConcatArray<ChatMessage> }) => {
      ...
      >
        {chats.length !== 0
          ? chats.map((chat, index) => {
              return chat.from.username === session.session?.username ? (
                <Grid item key={index} xs={12} container>
                  <Grid item xs={3} />
                  <Grid item xs={9}>
                    <Paper className={classes.chatBubbleRight}>
                ...
                <Grid item key={index} xs={12} container>
                  <Grid item xs={9}>
                    <Paper className={classes.chatBubbleLeft}>
                      <Typography variant="subtitle1" color="initial">
                        {chat.message}
                      </Typography>
                    </Paper>
